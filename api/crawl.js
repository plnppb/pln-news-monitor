...
export default async function handler(req, res) {
  ...
  // Verifikasi secret key agar tidak bisa dipanggil sembarang orang
  const secret = req.headers['x-cron-secret'] || req.query.secret;
  if (secret !== process.env.CRON_SECRET) {
    return res.status(401).json({ error: 'Unauthorized' });
  }

  const { feeds, keyword = 'PLN Papua' } = req.body || {};

  if (!feeds || !feeds.length) {
    return res.status(400).json({ error: 'No feeds provided' });
  }

  const results = { total: 0, saved: 0, errors: [] };

  // Proses max 20 feed per request agar tidak timeout
  const batch = feeds.slice(0, 20);

  await Promise.allSettled(batch.map(async (feedUrl) => {
    try {
      ...
      results.errors.push({ feed: feedUrl, error: e.message });
    }
  }));
